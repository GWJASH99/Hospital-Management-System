...
const CompletedOrders = () => {
  ...
  return (
    <div className={`p-6 md:p-10 font-sans space-y-8 min-h-full flex flex-col ${dark ? 'text-gray-100' : 'text-gray-900'}`}>
      <Search title={tObj?.completedOrdersTitle || "Completed Orders"} />
        ...
        <div className="grid gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {completedMedicines.length > 0 ? (
            completedMedicines.map((medicine) => (
              <div
                key={medicine._id}
                className={`group flex flex-col justify-between p-6 rounded-3xl border transition-all duration-300 hover:-translate-y-1 hover:shadow-xl relative overflow-hidden ${dark ? 'bg-gray-900 border-gray-800 hover:shadow-black/50 hover:border-green-500/50' : 'bg-white border-gray-100 shadow-sm hover:shadow-green-500/10 hover:border-green-200'}`}
              >
                {/* Decorative Background Blob */}
                <div className={`absolute -right-10 -top-10 w-32 h-32 rounded-full opacity-10 transition-transform duration-500 group-hover:scale-150 ${dark ? 'bg-green-500' : 'bg-green-600'}`}></div>

                <div className="relative z-10 space-y-4 flex-1">
                  <div className="flex justify-between items-start">
                     <span className={`px-3 py-1 rounded-full text-xs font-bold ${dark ? 'bg-green-500/20 text-green-400' : 'bg-green-100 text-green-700'}`}>
                        {medicine.status}
                     </span>
                  ...
                  <h2 className="text-2xl font-black tracking-tight text-green-600 dark:text-green-400 line-clamp-2">
                    {medicine.userName}
                  </h2>
            ...
            <div className="col-span-full py-16 flex flex-col items-center justify-center text-gray-500 dark:text-gray-400 border-2 border-dashed rounded-3xl dark:border-gray-800 border-gray-200 h-full">
               ...
               <h3 className="text-xl font-semibold">{tObj?.noRecordsFoundText || "No Completed Orders Found."}</h3>
               <p className="mt-2 text-sm max-w-sm text-center">There are no completed medicine orders in the records.</p>
            </div>
          )}
        </div>
      </div>
